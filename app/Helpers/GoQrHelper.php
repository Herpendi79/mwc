...
if (!function_exists('generateGoQrAndSave')) {
    ...
    {
        //$path = base_path('../public_html/uploads/qrcode');
        ...
        try {
            ...
            if ($qrImage === false || $httpStatus !== 200) {
                Log::error("Failed download QR: HTTP {$httpStatus}, error: {$error}");
                throw new Exception("Failed download QR from GoQR API.");
            }

            file_put_contents($filePath, $qrImage);
            Log::info("QR Code success saved at : {$filePath}");
