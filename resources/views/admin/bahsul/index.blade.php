                        ...
                        <div class="flex items-center gap-4 w-full md:w-auto">
                            ...
                            <div class="relative w-full md:w-72" x-data="{ search: '{{ request('search') }}' }">
                                ...
                                <input type="text" x-model="search"
                                    @input.debounce.500ms="window.location.href = '{{ route('admin.bahsul.index') }}?search=' + encodeURIComponent(search)"
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                                    @foreach ($bahsul as $index => $item)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            ...
                                            <td class="p-4">{{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}
                                            </td>
                                            ...
                                            <td class="p-4 text-center">
                                                <div class="flex justify-center gap-2">
                                                    @if ($item->status == 'draft' || $item->status == 'arsip')
                                                        <form action="{{ route('admin.bahsul.status', $item->id_bs) }}"
                                                            method="POST"
                                                        ...
                                                        <form action="{{ route('admin.bahsul.hapus', $item->id_bs) }}"
                                                            method="POST"
