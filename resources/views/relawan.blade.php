                ...
                <div class="row">
                    ...
                    <div class="col-lg-8 mb-5 mb-lg-0">
                        ...
                        <div class="blog_left_sidebar">
                            @php
                                $folderPath = public_path('storage/foto_relawan');
                                ...
                                @foreach ($dataBahsul as $index => $bahsul)
                                    <article class="blog_item mb-5">
                                            ...
                                            @php
                                                $currentPhoto =
                                                    $totalPhotos > 0
                                                        ...
                                                        : 'relawan-default.jpeg';
                                            @endphp

                                            <img class="card-img rounded-0"
                                                src="{{ asset('storage/foto_relawan/' . $currentPhoto) }}"
                                                alt="{{ $bahsul->tema }}"
                                                style="width: 100%; height: 375px; object-fit: cover; display: block;">

                                            <a href="{{ route('relawan.detil', $bahsul->id_re) }}" target="_blank"
                                        ...
                                        <div class="blog_details">
                                            <a class="d-inline-block" href="{elawan' , $bahsul->id_re) }}" target="_blank">
                                                <h2>{{ $bahsul->judul }}</h2>
                                            </a>
                                            ...
                                            <div class="flex flex-wrap items-center justify-between gap-4 mt-4">
                                                <ul class="blog-info-link mb-0">
                                                    <li><a href="{elawan' , $bahsul->id_re) }}" target="_blank"><i
                                                                class="fa fa-user"></i>
                                                            {{ $bahsul->koordinator }}</a></li>
                                                    <li><a href="{elawan' , $bahsul->id_re) }}" target="_blank"><i
                                                ...
                                                <div>
                                                    @if (\Carbon\Carbon::parse($bahsul->tgl)->endOfDay()->isFuture())
                                                        <button type="button"
                                                            @click="openModal = true; selectedBahsulId = '{{ $bahsul->id_re }}'"
                                    ...
                                    <div x-show="openModal && selectedBahsulId === '{{ $bahsul->id_re }}'"
                                        style="display: none;"
                                        ...
                                        <div @click.outside="openModal = false"
                                            ...
                                            <form action="{{ route('relawan.daftar', $bahsul->id_re) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="id_re" x-model="selectedBahsulId">
                    ...
                    <div class="col-lg-4">
                        <div class="blog_right_sidebar">
                            ...
                            <aside class="single_sidebar_widget post_category_widget">
                                ...
                                <ul class="list cat-list">
                                    @foreach ($kategoriList as $k)
                                        <li>
                                            <a href="{{ route('relawan', ['keyword' => $k->judul]) }}" class="d-flex">
                                                <p>{{ $k->judul }}</p>
                                                <p>({{ $k->total }})</p>
                                            </a>
                                        </li>
                                ...
                                <div class="relawan-vertical-active">
                                    @foreach ($recentPosts as $index => $post)
                                        <div class="media post_item"
                                            ...
                                            <img src="{{ asset('storage/foto_relawan/' . $photoToDisplay) }}"
                                                alt="post"
                                                style="width: 80px; height: 80px; object-fit: cover; margin-right: 15px; border-radius: 4px;">

                                            <div class="media-body">
                                                <a href="{elawan' , $bahsul->id_re) }}" target="_blank">
                            ...
                            <aside class="single_sidebar_widget instagram_feeds">
                                <h4 class="widget_title">Galery Relawan Banjir</h4>
                                ...
                                <ul class="instagram_row"
                                    ...
                                    @foreach ($dataBahsul as $item)
                                        @php
                                            // Cek apakah kolom foto ada isinya, jika ya split dengan ';'
                                            $photos = !empty($item->foto) ? explode(';', $item->foto) : [];
                                        @endphp

                                        @foreach ($photos as $foto)
                                            @php
                                                $trimmedFoto = trim($foto);
                                                // Validasi: Pastikan nama file tidak kosong DAN file fisiknya benar-benar ada di storage
                                                $isExist =
                                                    !empty($trimmedFoto) &&
                                                    Storage::disk('public')->exists('foto_relawan/' . $trimmedFoto);

                                                // Tentukan path gambar akhir (jika ada gunakan file aslinya, jika tidak arahkan ke default)
                                                $imageSrc = $isExist
                                                    ? asset('storage/foto_relawan/' . $trimmedFoto)
                                                    : asset('storage/foto_relawan/relawan-default.jpeg');
                                            @endphp

                                            @if ($count < $maxPhotos)
                                                <li style="width: 90px; height: 90px;">
                                                    <a href="{{ route('relawan.detil', $item->id_re) }}" target="_blank">
