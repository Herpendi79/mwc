...
<aside id="main-sidebar"
    ...
    <nav class="flex-1 px-4 space-y-2 overflow-y-auto mt-4">
        <a href="{{ route('reviewer.index') }}"
            class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('reviewer.index') ? 'text-white bg-black dark:bg-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-800' }} rounded-xl font-semibold transition-all">
            <i class="ri-dashboard-fill text-xl"></i>
            <span>Dashboard</span>
        </a>

        <a href="{{ route('reviewer.conferences') }}"
            class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('reviewer.conferences') ? 'text-white bg-black dark:bg-white dark:text-black' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-800' }} rounded-xl font-semibold transition-all">
